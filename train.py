"""
ARC — Training Script v3
Improvements:
  1. 110 companies (50 distressed, 60 healthy)
  2. Expanded XBRL tags (fixes Altman Z dashes)
  3. Temporal train/test split (pre-2020 train, 2020+ test)
  4. Platt scaling calibration
  5. Altman Z baseline comparison
  6. Always saves LightGBM (required for SHAP TreeExplainer)
  7. Tuned LightGBM hyperparameters
...
# ── Fetch ─────────────────────────────────────────────────────────────────────
print("=" * 60)
print("  ARC Financial Distress Model — Training v3")
print("=" * 60)
...
# Train: pre-2020  |  Test: 2020+ (out-of-sample temporal validation)
...
# Fallback: if test set is too small, use company-level split
if df_te["distress"].sum() < 5 or (df_te["distress"]==0).sum() < 5:
    print("  ⚠️  Temporal split too sparse — falling back to company-level split")
...
z_te  = X_te[:, z_col]
# Z < 1.81 = distress prediction
z_pred = (z_te < 1.81).astype(int)
valid  = ~np.isnan(z_te)
if valid.sum() > 0:
    z_auc = roc_auc_score(y_te[valid], -z_te[valid])   # lower Z = more distress
...
# ── Platt scaling calibration on LightGBM ────────────────────────────────────
print("\nStep 7: Calibrating LightGBM with Platt scaling...")
from sklearn.linear_model import LogisticRegression as _LR

lgbm_model = RESULTS["LightGBM"]["model"]

# Platt scaling: held-out 20% of TRAINING data for calibration
# Never use test set for calibration — causes data leakage
from sklearn.model_selection import train_test_split as _tts
X_tr_main, X_tr_cal, y_tr_main, y_tr_cal = _tts(
    X_tr, y_tr, test_size=0.20, random_state=SEED, stratify=y_tr
)
# Re-fit LightGBM on the 80% portion only
lgbm_model.fit(X_tr_main, y_tr_main)
...
cal_proba = PlattCalibrator(lgbm_model, platt_lr).predict_proba(X_te)[:, 1]
cal_auc   = roc_auc_score(y_te, cal_proba)
cal_brier = brier_score_loss(y_te, cal_proba)

# PlattCalibrator imported from app.core.model_store
...
print(f"{'='*60}")
print(f"  Best overall : {best} (AUC {RESULTS[best]['test_auc']:.4f})")
print(f"  Saved model  : LightGBM (AUC {lgbm_res['test_auc']:.4f}, "
      f"F1 {lgbm_res['f1']:.4f})")
if altman_baseline:
    improvement = (lgbm_res['test_auc'] - altman_baseline['auc']) * 100
    print(f"  vs Altman Z  : +{improvement:.1f}% AUC improvement")
print(f"  Brier score  : {cal_brier:.4f} (calibrated)")
print(f"  Companies    : {df_raw['Ticker'].nunique()}")
